...
sl_si91x_request_tx_test_info_t tx_test_info = {
  ...
  .power                  = 0,  ///< tx test mode power.  Range : 2 - 18 dBm.
  .rate                   = 0,   ///< tx test mode rate
  .length                 = 0, ///< tx test mode length. Range: [24 - 1500] bytes in Burst mode and [24 - 260] bytes in Continuous mode
  .mode                   = 0, ///< tx test mode mode.  0 - Burst Mode; 1 - Continuous Mode; 2 - CW Mode; 3 - CW Mode center frequency - 2.5MHz; 4 - CW Mode center frequency + 5MHz
  .channel                = 0,     ///< tx test mode channel
  .rate_flags             = 0,  ///< tx test mode rate_flags
  .channel_bw             = 0,  ///< tx test mode tx test_ch_bw
  .aggr_enable            = 0, ///< tx test mode aggr_enable
  .reserved               = 0,    ///< tx test mode reserved
  .no_of_pkts             = 0,  ///< tx test mode no_of_pkts
  .delay                  = 0,       ///< tx test mode delay
  #if defined(SLI_SI917) || defined(DOXYGEN)
  .enable_11ax            = 0, ///< 11AX_ENABLE 0-disable, 1-enable
  .coding_type            = 0, ///< Coding_type 0-BCC 1-LDPC
  .nominal_pe             = 0,  ///< Indicates Nominal T-PE value. 0-0Us 1-8Us 2-16Us
  .ul_dl                  = 0, ///< Indicates whether the PPDU is UL/DL. Set it to 1 if PPDU is to be sent by station to AP; 0 if PPDU is to be sent by AP to station.
  .he_ppdu_type           = 0, ///< he_ppdu_type 0-HE SU PPDU, 1-HE ER SU PPDU, 2-HE TB PPDU, 3-HE MU PPDU
  .beam_change            = 0, ///< Indicates the spatial mapping of pre-HE and HE fields. Enter 0 for pre-HE and HE fields are spatially mapped in the same way and 1 for pre-HE and HE fields are spatially mapped differently.
  .bw                     = 0,    ///< Indicates the BW for the PPDU: 0 for 242-tone RU, 1 for upper 106-tone RU.
  .stbc                   = 0, ///< Indicates whether STBC is used for PPDU transmission. Set to 0 for no STBC and 1 for STBC (only if DCM field is set to 0).
  .tx_bf                  = 0, ///< Indicates whether beamforming matrix is applied to the transmission. 0 - no beamforming matrix, 1 - beamforming matrix.
  .gi_ltf                 = 0,        ///< Indicates the GI and LTF size. GI_LTF shall be in the range 0-3
  .dcm                    = 0,           ///< Indicates whether DCM is applied to Data Symbols. 0 - No DCM, 1 - DCM.
  .nsts_midamble          = 0, ///< Indicates the NSTS and Midamble Periodicity. NSTS_MIDAMBLE shall be in the range 0-7
  .spatial_reuse          = 0, ///< spatial_reuse shall be in the range 0-15. 4 indicates that spatial reuse is allowed during the transmission of PPDU.
  .bss_color              = 0,              ///< Color value of BSS. Must be in the range 0 to 63
  .he_siga2_reserved      = 0,     ///< HE_SIGA2_RESERVED shall be in the range 0-511
  .ru_allocation          = 0,          ///< Indicates the RU Allocation Subfield for 20MHz BW. Must be in the range 0-255.
  .n_heltf_tot            = 0,            ///< Indicates the number of HE-LTF to be transmitted. Can be in the range 0-7.
  .sigb_dcm               = 0,              ///< Indicates whether DCM is applied to SIG-B Symbols. 0-disable, 1-enable
  .sigb_mcs               = 0,             ///< Indicates the MCS for SIG-B Symbols. Allowed range is 0-5.
  .user_sta_id            = 0,           ///< Indicates the Station ID of the intended user. Allowed range is 0-2047.
  .user_idx               = 0,               ///< USER_IDX shall be in the range 0-8
  .sigb_compression_field = 0, ///< SIGB_COMPRESSION_FIELD shall be 0/1
  #endif
  };
...
sl_status_t sl_wifi_transmit_test_start_command_handler(console_args_t *arguments)
{
  ...
  tx_test_info.rate_flags    = GET_OPTIONAL_COMMAND_ARG(arguments, 5, 0, const uint16_t);
  tx_test_info.aggr_enable   = GET_OPTIONAL_COMMAND_ARG(arguments, 6, 0, const uint16_t);
  tx_test_info.no_of_pkts    = GET_OPTIONAL_COMMAND_ARG(arguments, 7, 0, const uint16_t);
  tx_test_info.delay         = GET_OPTIONAL_COMMAND_ARG(arguments, 8, 0, const uint32_t);
#if defined(SLI_SI917) || defined(DOXYGEN)
  tx_test_info.enable_11ax   = GET_OPTIONAL_COMMAND_ARG(arguments, 9, 0, const uint8_t);
  tx_test_info.he_ppdu_type  = GET_OPTIONAL_COMMAND_ARG(arguments, 10, 0, const uint8_t);
  tx_test_info.ru_allocation = GET_OPTIONAL_COMMAND_ARG(arguments, 11, 0, const uint8_t);
